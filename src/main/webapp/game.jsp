	...
	<div>
		<div class="container">
			<div class="info">
				...
				<div>
					...
					<div id="soldier-actions">
						...
						<div class="soldier-row">
							<span>Soldat [<%=pos%>]; defense = <%=map.getGrid()[i][j].getSoldier().getDefensePoint()%>/<%=map.getGrid()[i][j].getSoldier().getMaxDefensePoint()%>
								:
							</span>
							...
							%><button type="button"
								onclick="setActionSoldier('top', '<%=pos%>')">Haut</button>
							<%
							}
							%>
							<%
							if (map.canMove(i + 1, j)) {
							%><button type="button"
								onclick="setActionSoldier('bottom', '<%=pos%>')">Bas</button>
							<%
							}
							%>
							<%
							...
							%><button type="button"
								onclick="setActionSoldier('right', '<%=pos%>')">Gauche</button>
							<%
							}
							%>
							<%
							if (map.canMove(i, j + 1)) {
							%><button type="button"
								onclick="setActionSoldier('left', '<%=pos%>')">Droite</button>
							<%
							}
							%>
							<%
							...
							%><button type="button"
								onclick="setActionSoldier('forage', '<%=pos%>')">Forage</button>
							<%
							}
							%>
							<%
							...
							%><button type="button"
								onclick="setActionSoldier('heal', '<%=pos%>')">Soin</button>
							<%
							}
							}
							%>
					...
					<div>
						...
						<div id="city-actions">
							...
							<div class="city-row">
								...
								%><button type="button"
									onclick="setActionCity('former', '<%=pos%>')">Former
									une troupe</button>
								<%
								}
								%>
				...
				<button type="submit" onclick="setAction('endTurn')">Fin de
					tour</button>
				<%
				}
				%>
				<button type="submit" onclick="setAction('scoreGame')">Afficher
					le score</button>
			</div>
			<div class="grid">
				<%
				...
				<div class="<%=grid[i][j].getCellCSSClass()%>">
					<div class="image-container ">
						...
						<img src="<%=grid[i][j].getImagePath()%>"
							class="background-image" />
						<%
						}
						if (grid[i][j].isSoldier()) {
						%>
						<img src="<%=grid[i][j].getOverlayImagePath()%>"
							class="overlay-image" />
						<%
						}
						%>
